  ...
  componentWillMount(){
    ...
    axios
        ...
        await response.data.map(value=>{
          rmcap.push([(Date.parse(value.dateis)+"T00:00:00").toString(), -value.rank_mcap]);
          rchg.push([(Date.parse(value.dateis)+"T00:00:00").toString(), -value.rank_change]);
          rratio.push([(Date.parse(value.dateis)+"T00:00:00").toString(), value.rank_ratio.toFixed(4)]);
        })
          this.setState({
            fullData: [{
              name: 'RANK- MCap',
